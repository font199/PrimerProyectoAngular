...
import { PERSONAS } from '../../persona/shared/mock-persona';
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        return Observable.of(null).mergeMap(() => {
            ...
            if (request.url.match(/\/api\/personas\/\d+$/) && request.method === 'PUT') {
                // get new persona object from post body
                ...
                personas.forEach((persona: Persona) => {
                    if(persona.id  === id){
                        ...
                        persona.mail = updatePersona.mail;
            ...
            // delete persona
            if (request.url.match(/\/api\/persona\/\d+$/) && request.method === 'DELETE') {
                // check for fake auth token in header and return tarea if valid, this security is implemented server side in a real application
                //if (request.headers.get('Authorization') === 'Bearer fake-jwt-token') {
                    // find persona by id in personas array
                    let urlParts = request.url.split('/');
                    let id = urlParts[urlParts.length - 1];
